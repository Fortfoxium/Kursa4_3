...
procedure CheckUsers(Changebtn, AddButton, deletebutton:tbutton);
...
begin
   ...
   Changebtn.hint:='Вы главный регистратор, у вас нет прав менять данные';
   AddButton.Enabled:=false;
   AddButton.hint:='Вы главный регистратор, у вас нет прав менять данные';
   deletebutton.Enabled:=false;
   deletebutton.hint:='Вы главный регистратор, у вас нет прав менять данные';
end;
end;
procedure connection(DBase: TIBDatabase;IBTransaction: TIBTransaction;Query:tIBQuery);
var
...
DBase.Databasename:=(ini.readstring('Connection','Path','Ошибка'));
...
begin
    ...
    case index of
    1:begin
       case application.MessageBox('Вы уверены что хотите изменить эту запись?','Подтверждение',Tip)of
       IDYES:         //Предложение изменить выбранную запись
        Begin
         QueryString:='UPDATE '+Table.Name+' SET ';   //динамически определяем таблицу
         QueryString:=QueryString+((Table.Fields[0].Fieldname)+'='+quotedStr(Dataar[0])+', ');
         for i := 1 to Table.fieldCount-1 do        //цикл заполнения запроса
          begin
          ...
          Q.SQL.Add(QueryString);     //отправляем запрос и закрепляем данные
          Q.ExecSQL;
          q.close;
          Table.close;                  //Закрываем Query
          trans.commit;
        End;
       IDNO:;
       end;
    ...
    2:begin
       case application.MessageBox('Вы уверены что хотите удалить эту запись?','Подтверждение',Tip)of
       IDYES:
         Begin
    ...
    3:begin
       case application.MessageBox('Вы уверены что хотите добавить эту запись?','Подтверждение',Tip)of
       IDYES:
          Begin
