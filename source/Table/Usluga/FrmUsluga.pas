...
begin
if  AddBtn.Caption='Отмена' then  // если надпись на кнопке = отмена то
Begin
  lastbtn.Enabled:=true;
  Firstbtn.Enabled:=true;        //блокируем все поля и кнопку сохранения
  Search.Enabled:=true;          //Разблокировать все кнопки кроме сохранения
...
MED_USL.Enabled:=False;
  ...
  readdate;
End
  Else                        //иначе если надпись на кнопке не = отмена то
  begin
  lastbtn.Enabled:=false;
  Firstbtn.Enabled:=false;      // заблокировать все кнопки  кроме сохранения
  Search.Enabled:=false;
  Nextbtn.Enabled:=false;       // заблокировать поиск
  Backbtn.Enabled:=false;
  MED_USL.Enabled:=True;
  SaveBtn.Enabled:=true;
  DeleteBtn.Enabled:=False;
  ChangeBtn.Enabled:=False;
  AddBtn.Caption:='Отмена';
  USLUGA.last;
  readdate;
  query.sql.Clear;
  query.sql.add('SELECT GEN_ID( USLUGAgen, 0) from RDB$DATABASE');
  query.open;
  ID_MED_USL.text:=inttostr(1+(query.fields[0].asinteger));   //Добавляем +1 к ID в поле
  MED_USL.text:='';
  end;
end;
...
begin
if  ChangeBtn.Caption='Отмена' then // если надпись на кнопке = отмена то
Begin
MED_USL.Enabled:=False;         //блокируем все поля и кнопку сохранения
  SaveBtn.Enabled:=False;
  Search.Enabled:=true;
  DeleteBtn.Enabled:=True;
  AddBtn.Enabled:=True;
  ChangeBtn.Caption:='Изменить';
End
  Else
  begin                    //иначе если надпись на кнопке не = отмена то
    MED_USL.Enabled:=True; // заблокировать все кнопки  кроме сохранения
  DeleteBtn.Enabled:=False;// заблокировать поиск
  Search.Enabled:=false;
  AddBtn.Enabled:=False;
  ChangeBtn.Caption:='Отмена';
  end;
  end;

procedure TUsluga.DeleteBtnClick(Sender: TObject);
begin
MBox(2,query, USLUGA, IBTransaction, Data); //вызов процедуры для удаления
with usluga do
begin
SQL.clear;
...
open;
first;
end;                      //овновляем поля на форме
readdate;
end;

procedure TUsluga.SearchChange(Sender: TObject);
begin
try                           //при изменении поля попробывать
with  USLUGA do
begin
SQL.clear;                    //запрос на поиск похожих результатов
SQL.Add('SELECT * FROM USLUGA WHERE MED_USL LIKE ' +quotedstr('%'+Search.text+'%'));
open;
end;
readdate;                       //обновление данных
except
end;
end;
...
begin
if (ID_MED_USL.text='') or (MED_USL.text='') then ShowMessage('Одно из полей не заполнено')
else                                       //Если поля пустые вывод ошибки
begin                                      //Если не пустые то определяем было изменение или добавление
updates;                                   //заполняем массив данными
if ChangeBtn.Caption='Отмена' then         //Отправляем массив даннных в процедуру
  begin
   MBox(1,query, USLUGA,IBTransaction, Data);
  end;

if AddBtn.Caption='Отмена' then
  begin
   MBox(3,query, USLUGA,IBTransaction, data);
  end;
  With usluga do                  //обновляем данные в полях
  begin
   SQL.clear;
   SQL.Add('SELECT * FROM USLUGA Order by ID_Med_Usl ASC');
   open;
