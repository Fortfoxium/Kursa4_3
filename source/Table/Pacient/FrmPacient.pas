...
Begin
   ...
   if varisnull(PACIENT.FieldByName('EMAIL').Value)then EMAIL.Text:='Не указан'
   ...
   if PACIENT.FieldByName('POL').Value='М' then Radiogroup1.itemindex:=0
...
begin
  ...
  if Radiogroup1.itemindex=1 then   data[11]:='Ж'
   else data[11]:='М';
  data[12]:=APART.Text;
End;
...
begin
if (length(TEL.text)<13) or (length(OMS.text)<16) or (length(ID_CART.text)<5) then showmessage('Номер амбулаторной карты или ОМС или номер телефона, не достаточно длинные')
 else
 begin
  if (Fname.Text='') or (name.Text='') or (street.Text='') or (apart.Text='')or (ID_CART.Text='')or (TEL.Text='')or (OMS.Text='')or (building.Text='') or (otchname.Text='') then showmessage('Заполните все поля с *!') else
    begin
    if (date_born.Date>Now()) then showmessage('Вы из будущего?') else
      begin
        if (date_born.Date>IncYear(now(),-18)) then showmessage('Пациенту нет 18 лет!') else
        begin
          begin
            updates;
            if ChangeBtn.Caption='Отмена' then
              begin
