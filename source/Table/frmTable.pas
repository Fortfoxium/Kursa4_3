    ...
    with query do
      begin
        ...
        case key of
          ...
          3:
          begin
            sql.add('SELECT ID_PACIENT AS "Номер пациента", FNAME AS "Фамилия пациента",NAME AS "Имя пациента",'
            +'OtchNAME AS "Отчество пациента",Date_Born AS "Дата рождения", ID_CART AS "Номер амбулаторной карты",'
             +'EMAIL AS "Электронная почта пациента", TEL AS "Телефон пациента", OMS AS "ОМС пациента", STREET AS'
             +'"Улица проживания пациента", BUILDING AS "Номер дома", POL AS "Пол пациента", APART AS "Номер квартиры"'
          ...
          5:
          begin
            sql.add('SELECT CAB AS "Номер кабинета", NAME_CAB AS "Название кабинета", Id_doctor AS "ID врача", Id_med_usl AS "ID мед. услуги" FROM CABINET Order by CAB ASC');
            open;
          end;
    end;
   n:=query.FieldCount;
