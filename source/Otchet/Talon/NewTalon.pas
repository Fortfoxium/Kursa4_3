...
const
  week:array[1..7] of string = ('Воскресенье','Понедельник','Вторник','Среда','Четверг','Пятница','Суббота');
var
...
if (Fname.text='') and (Otchname.text='') and (namepac.text='')then  begin
  ...
  With QUERY do
    ...
    if QUERY.IsEmpty then statusbar1.Panels[0].Text:='Такого пациента нет в базе данных' else  statusbar1.Panels[0].Text:='Такой пациент есть в базе данных'
...
if (not(NamePac.text='')) or (not(fName.text='')) or (not(otchName.text='')) then
    ...
    Tip:=MB_YESNO;
 if (NamePac.text='') or (FName.text='') or (OtchName.text='') or (ID_MED_USL.text='') or (DAYS.text='') or (Hours.text='') then showmessage('Одно из обязательных полей не заполнено')
 else
 begin
 With QUERY do
  begin
  ...
  if Query.isEmpty then
    begin
      case application.MessageBox('Такого пациента нет. Вы можете добавить нового пациента. Сделать это?','Подтверждение',Tip) of
           IDYES:
              Begin
                openNewPacient(SELF);
              End;
      ...
      ID_CAb:=QUERY.Fields[0].AsInteger;
       ...
       With QUERY do
        begin
         ...
         if not(query.IsEmpty) then Showmessage('Такая запись уже существует')
         Else
         begin
          With QUERY do
            begin
          ...
          trans.active:=true;
          trans.commit;
          case application.MessageBox('Талон создан, хотите вывести его в Word?','Подтверждение',Tip) of
           IDYES:
            Begin
              App:=CreateOleObject('Word.Application'); //запустили
              App.visible:=false;
              OpenDoc('.\..\doc\Шаблоны\Талон.docX');
              StartOfDoc; while not
              FindAndPasteTextDoc('###Фамилия& ',Fname.text) do;
              StartOfDoc; while not
              FindAndPasteTextDoc('###Имя&',namepac.text) do;
              StartOfDoc; while not
              FindAndPasteTextDoc('###Отчество&',otchname.text) do;
              StartOfDoc; while not
              ...
              StartOfDoc; while not
              FindAndPasteTextDoc('###Кабинет&',Name_CAB) do;
              StartOfDoc; while not
              FindAndPasteTextDoc('###ДеньЗаписи&',strin) do;
              StartOfDoc; while not
              FindAndPasteTextDoc('###ВремяЗаписи&',hours.text) do;
              StartOfDoc; while not
              ...
              StartOfDoc; while not
              FindAndPasteTextDoc('###Услуга&',ID_MED_USL.text) do;
              StartOfDoc; while not
              FindAndPasteTextDoc('###Дата&',dateToStr(now())) do;
              if (savestring='') or (savestring='Ошибка') then
              begin
                                 try
                                 app.Activedocument.saveas(ExtractFilePath(Application.ExeName)+'.\..\DOC\Талоны\'+dateToStr(now())+' '+namePac.Text+' '+fname.Text+' '+otchname.Text+' '+ID_MED_USL.text+'.rtf');
                                finally
                                  showmessage('Указанный путь не найден, поэтому файл сохранён в стандартную папку: '+ExtractFilePath(Application.ExeName)+'.\..\DOC\Талоны\'+dateToStr(now())+' '+namePac.Text+' '+fname.Text+' '+otchname.Text+' '+ID_MED_USL.text+'.rtf')
                                end;
              end
              else
              begin
                                ...
                                try
                                 app.Activedocument.saveas(savestring+dateToStr(now())+' '+namePac.Text+' '+fname.Text+' '+otchname.Text+' '+ID_MED_USL.text+'.rtf');
                                  ...
                                  showmessage('Файл сохранён как: '+savestring+dateToStr(now())+' '+namePac.Text+' '+fname.Text+' '+otchname.Text+' '+ID_MED_USL.text+'.rtf')
                                end;
              end;
...
case application.MessageBox('Вы уверены что хотите добавить нового пациента?','Подтверждение',Tip)of
       IDYES:
        Begin
        openNewPacient(SELF);
        End;
