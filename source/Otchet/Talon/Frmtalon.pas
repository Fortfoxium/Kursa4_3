              ...
              OpenDoc('.\..\doc\Шаблоны\Талон.docX');
                           ...
                           StartOfDoc; while not
                            FindAndPasteTextDoc('###Дата&',dateToStr(Datatalon.date)) do;
                              Tip:=MB_YESNO;
                             case application.MessageBox('Хотите сохранить этот файл на компьютере?','Сохранение',Tip) of
                             IDYES:
                                Begin
                                ...
                                app.Activedocument.saveas(ExtractFilePath(Application.ExeName)+'.\..\DOC\Талоны\'+dateToStr(now())+' '+name+' '+fname+' '+otchname+' '+ID_MED_USL+'.rtf');
                                  ...
                                  showmessage('Указанный путь не найден, поэтому файл сохранён в стандартную папку: '+ExtractFilePath(Application.ExeName)+'.\..\DOC\Талоны\'+dateToStr(now())+' '+name+' '+fname+' '+otchname+' '+ID_MED_USL+'.rtf')
                                end;
                                   end else
                                   begin
                                     try              //
                                app.Activedocument.saveas(savestring+dateToStr(now())+' '+name+' '+fname+' '+otchname+' '+ID_MED_USL+'.rtf');
                                  ...
                                  showmessage('Файл сохранён как: '+savestring+dateToStr(now())+' '+name+' '+fname+' '+otchname+' '+ID_MED_USL+'.rtf')
                                end;
                                   end;
